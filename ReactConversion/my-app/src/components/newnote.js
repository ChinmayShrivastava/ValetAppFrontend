...
import {
  ...
  submitNote,
  setDocId
} from '../redux/features/noteSlice'; // Adjust the path as necessary
import { submitNoteAPI } from '../functions/content'; // Adjust the path as necessary
...
export default function NewNote() {
  ...
  const { type, docid, doctitle, title, url, content, topics } = noteReducer.value;

  const [documentTypes, setDocumentTypes] = useState('Article,Book,Video,Topic,None');
  ...
  useEffect(() => {
    ...
    if (id_) {
      dispatch(setDocId(id_));
    }
    if (name_) {
      dispatch(setDocTitle(name_));
    }
    if (url_ && url_ !== 'undefined') {
      dispatch(setUrl(url_));
    }
    if (type_) {
      dispatch(setType(type_));
    }
  }, [dispatch]);
  ...
  }, [dispatch, type, doctitle]);

  const handleSubmit = e => {
    e.preventDefault();
    submitNoteAPI(docid, content, topics, doctitle, type, url, title).then(res => {
      if (res[0]) {
        dispatch(
          submitNote({
            type: type,
  ...
  const handleDocumentTitleChange = e => {
    dispatch(setDocTitle(e.target.value));
  };
                ...
                <div className="drop-shadow-md">
                    <div className='flex flex-col justify-start p-8 bg-[#FAFAFA] rounded-lg'>
                        ...
                        <div className='flex flex-col'>
                            ...
                            <div className='flex flex-row justify-start align-middle px-2 rounded-md mr-2'>
                                {
                                    documenttypes.split(',').map((documenttype) => {
